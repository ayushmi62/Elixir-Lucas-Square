...
use GenServer
  ...
  def init(initial_data) do
    ...
    cond do
      Kernel.length(initial_data) == 2 ->
        ...
        Worker.spawnActors(0,m,n)                 #pass the arguments i.e., sequence length & N to the Worker
      true ->
  ...
  #assigns new starting point to calculate the squares from
...
defmodule Worker do
  ...
  #it finds if the sum of squares is perfect square or not
  def calc_square(count,size) do
    ...
    if (sqr - sqroot)>0 do
      ...
      IO.puts "#{count}"                          #prints the result
    end
  end
